...
(function() {
  var BacklogDB, CoffeeScript, Config, Crypto, DBMigrator, Fs, Path, SCHEMA_VERSION, Sqlite3, Squel, UUID, compact, count, del, ends, extend, flatten, last, merge, starts, _, _ref;
  ...
  BacklogDB = (function() {
    function BacklogDB(callback) {
      var migrationsDir;
      this.file = Path.join(Config.getDataDirectory(), 'backlog.db');
      migrationsDir = __dirname + '../../../db';
      this.db = new Sqlite3.Database(this.file, (function(_this) {
        return function() {
          var m;
          m = new DBMigrator({
            db: _this.db,
          ...
          return m.migrate(SCHEMA_VERSION, function() {
    ...
    BacklogDB.prototype.close = function() {
      return this.db.close();
    };

    BacklogDB.prototype.selectUsers = function(callback) {
    ...
    BacklogDB.prototype.selectUser = function(id, callback) {
      return this.db.get('SELECT * FROM users WHERE uid = $uid', {
        $uid: id
      }, function(err, row) {
        if (err) {
          throw err;
    ...
    BacklogDB.prototype.selectUserByName = function(name, callback) {
      return this.db.get('SELECT * FROM users WHERE name = $name', {
        $name: name
      }, function(err, row) {
        if (err) {
          throw err;
    ...
    BacklogDB.prototype.selectConnections = function(uid, callback) {
      return this.db.all('SELECT * FROM connections WHERE uid = $uid', {
        $uid: uid
      }, function(err, rows) {
        if (err) {
          throw err;
    ...
    BacklogDB.prototype.insertUser = function(name, password_hash, is_admin, callback) {
      var push_id, push_key, self;
      if (_.isEmpty(name)) {
        throw 'name is required';
      }
      if (_.isEmpty(password_hash)) {
        throw 'password hash is required';
      }
      push_id = UUID.v4();
      push_key = Crypto.randomBytes(32).toString('base64');
      self = this;
      return this.db.run("INSERT INTO users (name, password, is_admin, push_id, push_key)\nVALUES ($name, $password, $is_admin, $push_id, $push_key)", {
        $name: name,
        ...
        return self.selectUser(this.lastID, function(row) {
          return callback(row);
        });
      });
    };

    BacklogDB.prototype.insertConnection = function(uid, options, callback) {
      var self, _ref1;
      ...
      self = this;
      return this.db.run("INSERT INTO connections (uid, name, server, port, is_ssl, nick, user_name, real_name, server_pass)\nVALUES ($uid, $name, $server, $port, $is_ssl, $nick, $user_name, $real_name, $server_pass)", {
        $uid: uid,
    ...
    BacklogDB.prototype.updateUser = function(uid, options, callback) {
      var self, setAttribute, sql;
      self = this;
      sql = Squel.update().table('users');
      sql.where('uid = $uid');
      setAttribute = (function(_this) {
        return function(name, options, key) {
          if (_.has(options, key)) {
            return sql.set(name, options[key]);
          }
        };
      })(this);
      ...
      return this.db.run(sql.toString(), {
        ...
        return self.selectUser(uid, function(row) {
          return callback(row);
        });
      });
    };
    ...
    BacklogDB.prototype.deleteConnection = function(cid, callback) {
      ...
      return this.db.serialize((function(_this) {
        return function() {
    ...
    BacklogDB.prototype.insertBuffer = function(cid, uid, name, type, callback) {
      var autoJoin;
      autoJoin = type === 'channel';
      return this.db.run('INSERT INTO buffers (cid, uid, name, type, auto_join) VALUES ($cid, $uid, $name, $type, $auto_join)', {
        $cid: cid,
        $uid: uid,
        $name: name,
        $type: type,
    ...
    BacklogDB.prototype.deleteUser = function(uid, callback) {
      return this.db.run('DELETE FROM users WHERE uid = $uid', {
        $uid: uid
      }, (function(_this) {
        return function(err) {
          if (err) {
    ...
    BacklogDB.prototype.selectEventsRange = function(cid, bid, num, beforeid, callback) {
      var query;
      query = "SELECT eid, bid, data, created_at\nFROM events\nWHERE eid IN (\n    SELECT eid\n    FROM EVENTS\n    WHERE\n      bid = $bid AND\n      eid < $beforeid\n    ORDER BY eid DESC\n    LIMIT $limit\n)\nORDER BY eid ASC";
      return this.db.all(query, {
        $bid: bid,
    ...
    BacklogDB.prototype.getAllLastSeenEids = function(uid, callback) {
      var query;
      query = "SELECT cid, bid, last_seen_eid\nFROM buffers\nWHERE uid = $uid AND\nlast_seen_eid IS NOT NULL";
      return this.db.all(query, {
        $uid: uid
      }, function(err, rows) {
        var bid, cid, eid, result, row, _i, _len;
        if (err) {
          throw err;
        }
        result = {};
...
//# sourceMappingURL=backlog_db.map
