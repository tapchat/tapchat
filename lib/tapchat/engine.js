...
(function() {
  var B, BacklogDB, Buffer, CoffeeScript, Config, Connection, Daemon, Engine, Express, Fs, Http, PasswordHash, Path, Url, Util, WebSocket, WorkingQueue, compact, count, del, ends, extend, flatten, last, merge, starts, _ref;
  ...
  Engine = (function() {
    ...
    Engine.prototype.broadcast = function(message, cb) {
      var client, queue, _fn, _i, _len, _ref1,
        _this = this;
      queue = new WorkingQueue(this.clients.length);
      _ref1 = this.clients;
      _fn = function(client) {
    ...
    Engine.prototype.messageHandlers = {
      heartbeat: function(client, message, callback) {
        var bid, buffers, cid, eid, queue, seenEids, _fn,
          _this = this;
        ...
        for (cid in seenEids) {
          ...
          _fn = function(bid, eid) {
            return queue.perform(function(over) {
              return _this.db.setBufferLastSeenEid(bid, eid, over);
            });
          };
          for (bid in buffers) {
            eid = buffers[bid];
    ...
    Engine.prototype.say = function(client, message, callback) {
      var conn, text, to;
    ...
    Engine.prototype.join = function(client, message, callback) {
      var chan, conn;
    ...
    Engine.prototype.part = function(client, message, callback) {
      var conn;
      conn = this.findConnection(message.cid);
    ...
    Engine.prototype.disconnect = function(client, message, callback) {
      return this.findConnection(message.cid).disconnect(function() {
        return callback();
      });
    };

    Engine.prototype.reconnect = function(client, message, callback) {
      var _this = this;
    ...
    Engine.prototype['add-server'] = function(client, message, callback) {
      var _this = this;
    ...
    Engine.prototype['edit-server'] = function(client, message, callback) {
      var conn;
      conn = this.findConnection(message.cid);
      return conn.edit(message, callback);
    };

    Engine.prototype['delete-connection'] = function(client, message, callback) {
      var conn;
      conn = this.findConnection(message.cid);
