...
(function() {
  ...
  Connection = (function(_super) {
    ...
    function Connection(engine, options, callback) {
      ...
      this.engine.db.getBuffers(this.id, function(buffers) {
    ...
    Connection.prototype.disconnect = function(cb) {
      if (this.client && this.client.conn.readyState !== 'closed') {
    ...
    Connection.prototype.updateAttributes = function(options) {
      ...
      serverChanged = (this.getHostName() && (this.getHostName() !== options.server || this.getPort() !== options.port)) || (this.client.opt.secure !== (!!options.ssl));
      ...
      for (signalName in _ref1) {
        ...
        _results.push((function(signalName, signalHandler) {
          return _this.client.addListener(signalName, function() {
            ...
            return (_ref2 = _this.queue).perform.apply(_ref2, [handler].concat(__slice.call(arguments)));
          });
    ...
    Connection.prototype.signalHandlers = {
      ...
      registered: function(message, over) {
        ...
        for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
          buffer = _ref1[_i];
          if (buffer.type === 'channel' && buffer.autoJoin) {
      ...
      message: function(nick, to, text, message, over) {
        ...
        if (to.match(/^[&#]/)) {
          ...
          return this.getOrCreateBuffer(nick, Buffer.prototype.TYPE_QUERY, function(buffer) {
            return buffer.addEvent({
              type: 'buffer_msg',
              from: nick,
              msg: text,
      ...
      invite: function(channel, from, message, over) {
        return Connection.consoleBuffer.addEvent({
          type: 'channel_invite',
          channel: channel,
          from: from
        }, over);
