...
(function() {
  var B, Base64, Buffer, CoffeeScript, Config, Connection, DataBuffer, Log, PushClient, User, WorkingQueue, compact, count, del, ends, extend, flatten, last, merge, starts, _ref;
  ...
  User = (function() {
    function User(engine, info) {
      ...
      this.pushId = info.push_id;
      this.pushKey = new DataBuffer(info.push_key, 'base64');
      this.pushClient = new PushClient(this);
      this.engine.db.selectConnections(this.id, (function(_this) {
    ...
    User.prototype.edit = function(options, callback) {
      return this.engine.db.updateUser(this.id, options, (function(_this) {
        return function(row) {
    ...
    User.prototype.addClient = function(client, inbandBacklog) {
      ...
      client.onmessage = (function(_this) {
    ...
    User.prototype.send = function(client, message, cb) {
      ...
      client.sendQueue.perform((function(_this) {
    ...
    User.prototype.isBufferSelected = function(bid) {
      var client, _i, _len, _ref1;
      _ref1 = this.clients;
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        client = _ref1[_i];
        if (client.selectedBid === bid) {
          return true;
        }
      }
      return false;
    ...
    User.prototype.addConnection = function(options) {
      return new Connection(this, this.engine, options, (function(_this) {
    ...
    User.prototype.removeConnection = function(conn, cb) {
      return conn["delete"]((function(_this) {
        return function() {
    ...
    User.prototype.broadcast = function(message, cb) {
      ...
      if (!message.is_backlog) {
        if (message.highlight && message.bid && (!this.isBufferSelected(message.bid))) {
    ...
    User.prototype.messageHandlers = {
      heartbeat: function(client, message, callback) {
        ...
        if (message.selectedBuffer != null) {
          client.selectedBid = message.selectedBuffer;
        }
        if (message.seenEids != null) {
      ...
      reconnect: function(client, message, callback) {
        return this.findConnection(message.cid).reconnect((function(_this) {
          return function() {
            return callback();
          };
        })(this));
      ...
      'add-server': function(client, message, callback) {
        return this.engine.db.insertConnection(this.id, message, (function(_this) {
