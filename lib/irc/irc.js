    ...
    self.addListener("raw", function (message) { // {{{
        switch ( message.command ) {
            ...
            case "NICK":
                ...
                if ( self.opt.debug )
                    util.log("NICK: " + message.nick + " changes nick to " + newNick);
            ...
            case "rpl_topic":
                ...
                if ( channel ) {
                    ...
                    self.emit('topic', message.args[1], channel.topic);
                }
                break;
            case "rpl_notopic":
                var channel = self.chanData(message.args[1]);
...
Client.prototype.connect = function ( retryCount, callback ) { // {{{
    ...
    if (self.conn && self.conn.readyState != 'closed') {
        console.log('ignored attempt to connect when already connected! ' + self.conn.readyState);
        return;
    }
    ...
    if (self.opt.secure) {
        ...
        self.conn = tls.connect(self.opt.port, self.opt.server, creds, function() {
           ...
           if (self.conn.authorized ||
              ...
              util.log('TLS auth error: ' + self.conn.authorizationError);
           }
        });
    }else {
...
Client.prototype.disconnect = function ( message, callback ) { // {{{
    ...
    if (self.retryTimeout) {
      ...
      self.clearTimeout(self.retryTimeout);
      self.retryTimeout = null;
    }
    ...
    if (!self.conn) {
        console.log('disconnect called when no conn');
        if (typeof(callback) === 'function') {
            callback();
        }
        return;
    ...
    if (typeof(callback) === 'function') {
      console.log('GOT CLOSE CALLBACK');
      self.conn.once('close', callback);
    }
    self.conn.destroy();
...
Client.prototype.send = function(command) { // {{{
    ...
    if ( this.opt.debug )
        util.log('IRC SEND: ' + command + " " + args.join(" "));
